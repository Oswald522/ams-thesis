...
习惯使用Latex模板，奈何并未提供相关的Latex模板，花了两天时间按照Latex教程制作一份.建议考虑多种因素（个人及老师习惯、现有论文素材、熟悉程度等）进行判断衡量评价.
| 类型 | 优点说明  |缺点备注 |
|:--------| :---------|:--------|
|latex|  支持Git，修改版本较为方便，<br>目前不需要但是想保留的内容直接注释<br>公式排版漂亮，引用简单|需要编译，直接生成PDF，<br>修改不方便，需要Tex发行版或Overleaf|
|woord| 官方提供，可见即所得，<br>修改或者检查容易|版本修订麻烦<br>格式容易发生变化|
...
可用，但是不建议作为边写边看的工具.原因是overleaf虽然在线编译但是随着论文内容增加，编译时间增加（特别是没有会员的时候），可能会抵消latex格式调整的节约出的时间.需要进行的设置有 `菜单->编译器`选择`XeLatex`,主文档`main.tex`.
...
1. 一级标题和二级标题的段前段后间距在Word当中表达较为奇怪，目前为暂定设置.

## 修改历史

| 时间 | 修改说明  |备注 |
|:--------| :---------|:--------|
|2024/09/02|  按照提供的word模板，开始初版提交.|填充占位符|
|2024/09/15| 修改cls文件中关于浮动体（表格、图）的间距|填充占位符|
|2024/10/28| 完成模板提交|填充占位符|
